...
class BatchRenormalization2D(Module):
    ...
    def __init__(self, num_features,  gamma=None, beta=None,
        ...
        # self.r_max = torch.tensor(1.5, requires_grad=False)
        # self.d_max = torch.tensor(2.5, requires_grad=False)
    ...
    def forward(self, x):
        ...
        batch_ch_mean = torch.mean(x, dim=(0,2,3), keepdim=True).to(device)
        batch_ch_std = torch.clamp(torch.std(x, dim=(0, 2, 3), keepdim=True),

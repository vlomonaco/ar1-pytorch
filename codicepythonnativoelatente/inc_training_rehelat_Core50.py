...
def main_Core50(conf, run, close_at_the_end = False):
    ...
    train_utils.modify_net_prototxt("D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatRehaConv5_4dw21107_brn3.prototxt", "D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatRehaConv5_4dw21107_brn3b.prototxt")
    # train_utils.modify_net_prototxt("D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatRehaAvgPool21107_brn3.prototxt", "D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatRehaAvgPool21107_brn3b.prototxt")
    # train_utils.modify_net_prototxt("D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatRehaConv5_4dw6464_brn3.prototxt", "D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatRehaConv5_4dw6464_brn3b.prototxt")
    # train_utils.modify_net_prototxt("D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatRehaAvgPool6464_brn3.prototxt", "D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatRehaAvgPool6464_brn3b.prototxt")
    # train_utils.modify_net_prototxt("D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatReha.prototxt", "D:/PRJ/CNN/Experiments/Core50/NIC/NIC_MobileNetV1dw_froz_LatReha_bn.prototxt")
    ...
    for batch in range(initial_batch, batch_count):
        ...
        if batch == 0:
            ...
            cwr.set_brn_past_weight(solver.net, 20000)

        #only the first round
        if first_round:
            if batch == 1 and (conf['strategy'] in ['ewc','cwr', 'cwr+', 'syn', 'ar1','ar1free']):
        ...
        if conf['strategy'] in ['cwr+','ar1','ar1free'] and batch > initial_batch:  # in the first batch done before to correctly update stats
        ...
        # memoria temporanea per tracce reharsal
        ...
        while it < train_iterations[batch]:
            ...
            if reharshal_size > 0:
                ...
                #solver.net.blobs['data_reha'].data[...] = rehe_x[reha_start:reha_end].reshape((reha_in_minibatch, -1,1,1))
            ...
            # per batch successivi a 0 salva tracce per reharshal durante la prima epoca solamente
        ...
        # carica pesi da vecchio modello
        # if batch == 0:
        #    solver.net.copy_from(conf['tmp_weights_file'])

        # per batch 0 devo rifare forward alla fine
        if batch == 0:
            reha_it = 0
...
if __name__ == "__main__":
    ...
    conf_NIC = {
        ...
        'strategy': 'ar1free',  # 'naive','lwf','ewc'
        ...
        #'rehearsal_layer': 'pool6', # for CWR+
        'rehearsal_layer': 'conv5_4/dw', # reharsal level
        'rehearsal_stop_layer': 'conv5_4/sep', # reharsal pattern @ prev_level
        #'strategy': 'SST_A_D',
        'backend': 'GPU',
        'accuracy_max': 0.8,   # for plot        
    ...
    assert conf['strategy'] in ['naive','cwr+','ar1','ar1free'], "Strategy " + conf['strategy'] + " not supported!"
